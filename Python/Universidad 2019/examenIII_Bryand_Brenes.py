...
def crea_listas (lista):
    ...
    c = 0
    n = 0
    while c < len(lista) -1:#para que funcione eliminar el -1        
        ...
        #para que funciones todos los > y < se deben sustituir por >= y <=, excepto el del while
        elif c == len(lista)-1:
            anterior = lista[c-1]
            actual = lista[c]
...
def crear_dicc(lista,c,tamaño,d):
    if tamaño == c:
        ...
        j = lista[c]#Para que funcione j = lista[c][1]
        if j not in d:
            d[j] = []
...
def sacar_materias (lista,tamaño,c,n_list):#para que funcione se debe borrar tamaño de los datos de entrada
    if c == tamaño:#para que funcione sustituir tamaño por len(lista)
        return n_list
    else:
...
def sacar_materias_aux(lista,c,n_lista):
    if c == len(lista):
        ...
        return sacar_materias_aux(lista,c+1,n_lista)
...
def materias(diccionario):
    ...
    d = {}
    materias_lista = sacar_materias(lista_n,c,[]) #para que funcione sacar_materias(lista_n,len(lista_n),0,[])
    nuevo_d = crear_dicc(materias_lista,0,len(materias_lista),d)
    lista_f = lista_completa(carne,lista_n,0,[])
    return materias_aux(diccionario,lista_f,0)#para que funcione return materias_aux(d,lista_f,0)
...
def actualizar_dicc(diccionario,lista,c):
    ...
    if c == len(lista[1]): #Para que funcione hay que poner > en lugar de ==
        return diccionario
    else:
        materia = lista[c,1]
        estudiante = [(lista[0],lista[1],lista[c],[0])]
        
        # para que funcne hay que sustiruir lo anterior con
...
def lista_completa (lista1,lista2,c,lista):
    ...
    if c == len (lista1):
        ...
        j = [lista1[c]] + [lista2[2]]#pra que funcione j = [lista1[c]] + [lista2[c]]
        lista.append(j)
        return lista_completa (lista1,lista2,c,lista) #Para que funcione hay que poner c+1
