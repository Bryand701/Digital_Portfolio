...
def crea_listas (lista):
    ...
    while c < len(lista) -1:#para que funcione eliminar el -1        
        ...
        #para que funciones todos los > y < se deben sustituir por >= y <=
        elif c == len(lista)-1:
            anterior = lista[c-1]
            actual = lista[c]
...
def sacar_materias (lista,c,n_lista):
    if c == len(lista):
        return n_lista
    else:
        n_lista.append(lista[c][1])
        return sacar_materias (lista,c+1,n_lista)
        
def materias(diccionario):

    carnet = list(diccionario.keys())
    lista_n = list(diccionario.values())
    d = {}
    materias_lista = sacar_materias(lista_n,0,[])
    nuevo_d = crear_dicc(materias_lista,0,len(materias_lista),d)
    lista_f = lista_completa(carnet,lista_n,0,[])
